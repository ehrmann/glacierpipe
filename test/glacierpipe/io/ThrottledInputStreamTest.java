...
public class ThrottledInputStreamTest {
	...
	@Test
	public void testThrottledInputStreamByteReadStrategy() throws IOException {
		byte[] buffer = new byte[1024 * 512];
		random.nextBytes(buffer);
		...
		try (InputStream in = new ThrottledInputStream(new ByteArrayInputStream(buffer), new FixedThrottlingStrategy(1024 * 64))) {
			int read;
			while ((read = in.read()) >= 0) {
				out.write(read);
			}
	...
	@Test
	public void testThrottledInputStreamBlockReadStategy() throws IOException {
		byte[] readBuffer = new byte[8192];
		...
		try (InputStream in = new ThrottledInputStream(new ByteArrayInputStream(buffer), new FixedThrottlingStrategy(64 * 1024))) {
			int read;
			while ((read = in.read(readBuffer)) >= 0) {
				out.write(readBuffer, 0, read);
				chunks++;
